...
class InpaintGeneratorQuant(BaseNetwork):
    def __init__(self, args):  # 1046
        super(InpaintGeneratorQuant, self).__init__()
        ...
        self.middle = nn.Sequential(*[AOTBlockQuant(256, args.rates) for _ in range(args.block_num)])
    ...
    def forward(self, x, mask):
        ...
        x = torch.tanh(x)
        x = self.dequant(x)
        acts = torch.stack(self.activations) if self.activations else None
        self.activations = []
        return x, acts
...
class AOTBlockQuant(nn.Module):
    def __init__(self, dim, rates):
        super(AOTBlockQuant, self).__init__()
        self.rates = rates
        for i, rate in enumerate(rates):
            self.__setattr__(
                "block{}".format(str(i).zfill(2)),
    ...
    def forward(self, x):
        ...
        out = torch.cat(out, 1)
        out = self.fuse(out)
        mask = self.quant(my_layer_norm(self.dequant(self.gate(x))))
        mask = torch.sigmoid(mask)
        return self.ff.add(self.ff.mul(x, self.ff.add(self.quant(torch.tensor(1.0)), self.ff.mul(self.quant(torch.tensor(-1.0)), mask))), self.ff.mul(out, mask))
